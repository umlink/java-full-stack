...
| **刻意练习（Deliberate Practice）** | 在「刚好够不着但踮脚能碰到」的难度区练习，而不是重复已会的 | boot-server 实操永远比当前能力难半档：学完概念就落代码，落不动了说明该回头补 |
...
③ 落：按 10-实战产品蓝图 的映射表，把本讲技术点落进 boot-server（Code Review 模式）
④ 检：讲末的自检/面试题闭卷过一遍（出题官模式），错题进卡点日志
⑤ 复：进入下一阶段前，让 AI 抽查上一阶段（间隔重复）
```

各文件的角色：[docs/](docs/README.md) 是教材、[实战产品蓝图](docs/10-实战产品蓝图/README.md) 是练习册目录、[boot-server](boot-server/README.md) 是操场、[CLAUDE.md](CLAUDE.md) 是给 AI 立的规矩、[学习时间规划](docs/学习时间规划.md) 是日程表、本文是用法说明书。
...
| 40 min | boot-server 落地本次技术点 | Code Review 模式 |
...
3. **诚实记账**：进度只认自检闭卷通过和 boot-server 落地完成，不认「看完了」

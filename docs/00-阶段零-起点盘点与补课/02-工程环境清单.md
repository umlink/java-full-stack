...
- **版本基线**：本计划以 **Java 25 LTS** 为基线，别装成老版本的 8 / 11——各版本新特性（record / 模式匹配 / 虚拟线程 / Scoped Values）随[起步基础知识](05-起步基础知识.md)与阶段一第 2 讲展开，这里先认「25」这个数。
...
> **与 05 讲的分工**：本讲讲「构建工具怎么用」（冲突怎么仲裁、BOM 怎么锁版本、多模块怎么聚合）；`mvnrepository` 怎么搜依赖、坐标 GAV 怎么读、scope 怎么选——那是[起步基础知识](05-起步基础知识.md)第 4 节的「找、加、装」流程。
...
**答**：**标准结论**：两条规则——① **最短路径优先**（依赖树上离根近的版本赢）；② 路径等长时**先声明优先**（pom 里写在前的赢）。

**原理层**：Maven 解析依赖时构建整棵依赖树，按上述规则为每个 `groupId:artifactId` 选出唯一胜者版本——这决定了运行时的真实类路径。

**工程层**：
- 排查与解决：`mvn dependency:tree -Dverbose`（`-Dverbose` 才会显示被仲裁掉的候选版本及 `omitted for conflict` 原因），定位后用 `<exclusions>` 精准排除传递进来的旧版本；全家桶类依赖（Spring Boot 等）用 BOM（`dependencyManagement` + `scope=import`）统一锁版本。
- 与 npm 的关键差异：没有 lockfile 的绝对确定性，所以「BOM 锁版本 + CI 上全量构建验证」是逼近确定性的组合拳；更深层的事故形态是**同一 jar 不同版本共存导致的 `NoSuchMethodError`**——这类运行期错误只在调用了新版本才有的方法时才爆，编译期完全无感。

### Q2：JDK、JRE、JVM 三者是什么关系？为什么现在很少单独提 JRE 了？

**答**：**标准结论**：三者是包含关系 **JDK ⊃ JRE ⊃ JVM**——JVM 是「跑字节码的虚拟机」；JRE = JVM + 运行时类库，面向「只运行不开发」；JDK = JRE + 编译器（javac）+ 开发工具，面向开发。

**工程层**（为什么 JRE 提得少了）：① JDK 11 起 Oracle 不再单独发布 JRE 安装包（瘦身需求改用 `jlink`）；② 容器时代普遍直接用 JDK 镜像——「服务器只装 JRE」的运维习惯在退化。**完整版**（编译运行链路、字节码与 JIT）在[起步基础知识](05-起步基础知识.md)第 1 节与 Q5。

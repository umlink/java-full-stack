...
- **你要带走的一句话**：**注解是给程序读的标签，Bean 是 Spring 替你管的对象，JDK 是工具箱、JRE 是运行环境、JVM 是虚拟机，代码经 javac 编译成字节码再交给 JVM，pom 是项目的 package.json，依赖按坐标装进全局缓存**——识字课一次上完，后面看到这些不再停顿。
...
2. Bean：Spring 世界的「零件」（是什么、容器怎么管、生命周期简版）
3. JDK / JRE / JVM：Java 程序是怎么跑起来的（编译运行链路、一次编译到处运行）
4. Java 语法速览：代码长什么样（数据类型、面向对象、与前端差异最大的几个点、现代语法速览）
5. 一个 Java 项目长什么样：目录、包与入口（含 target/、jar、classpath、Javadoc）
6. 配置文件如何管理：谁管什么（pom / settings / application.yml / logback）
7. 依赖如何管理：找、加、装（坐标、中央仓库、本地缓存、scope）
...
### 3. JDK / JRE / JVM：Java 程序是怎么跑起来的

前两节看的是「贴标签」和「管对象」，这一节看「**跑起来**」——你写的 Java 代码到屏幕输出，中间隔着一条**编译 → 运行**的链路，链路上站着三个经常一起出现的缩写：**JDK、JRE、JVM**。
...
### 4. Java 语法速览：代码长什么样（识字版）

> 定位说明：这一节把「看到 Java 代码时不至于懵」的语法词过一遍——**不是系统语法课**（系统课在阶段一），而是「前端对照识字」：每个词标注了展开位置，现在只需对上脸、知道它 ≈ 前端的什么。

#### 4.1 数据类型：基本类型 vs 包装类型 vs String

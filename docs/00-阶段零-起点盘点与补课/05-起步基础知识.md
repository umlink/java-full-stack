...
- **你要带走的一句话**：**注解是给程序读的标签，Bean 是 Spring 替你管的对象，pom 是项目的 package.json，依赖按坐标装进全局缓存**——识字课一次上完，后面看到这些不再停顿。
...
4. 配置文件如何管理：谁管什么（pom / settings / application.yml / logback）
5. 依赖如何管理：找、加、装（坐标、中央仓库、本地缓存、scope）

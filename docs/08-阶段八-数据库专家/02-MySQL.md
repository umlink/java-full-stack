...
> **解释透一点（为什么索引能加速）**：
> - **B+ 树（B+ Tree）**：InnoDB 的索引，一种「矮胖」的多路搜索树——每个节点可存几百个键（扇出大），三层就能覆盖几千万行；2000 万行的表树高通常 3 层，2~3 次磁盘 IO 就定位到行；叶子节点用链表串成有序序列，范围查询顺着链表扫即可。
...
> **MVCC 与日志（面试高频）**：InnoDB 的 RR 防幻读靠「**MVCC（Multi-Version Concurrency Control，多版本并发控制）+ next-key 锁**」双保险。
> - **MVCC**：事务第一次读时生成 Read View（活跃事务快照），按可见性规则从版本链挑版本读——旧版本存在 **undo log**（撤销日志，保留旧版本供回滚与读快照）里，从而「读不加锁、读写不互斥」。
...
**底层原理**：
- B+ 树每个节点（16KB 页）可容纳上千个键，因此 2000 万行的表树高通常只有 3 层——从根到叶子 3 次磁盘 IO 就能定位一行。
...
**答**：**标准结论**：MVCC（多版本并发控制）让读写不互斥——读走快照、写走当前版本。InnoDB 的 RR 下用「MVCC 快照读 + next-key 锁（记录锁 + 间隙锁）」双保险防幻读。

**底层原理**：
- 每行有隐藏字段（事务 ID、回滚指针），更新时旧版本写进 undo log 形成版本链。
- 事务第一次读时生成 Read View，按可见性规则选版本，所以同一事务多次读结果一致（消除不可重复读）。
- RR 下对范围条件加 next-key 锁，把可能插入新行的间隙也锁住，幻读被堵死。

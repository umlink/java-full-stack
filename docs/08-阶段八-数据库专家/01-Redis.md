...
> **为什么是 RDB / AOF 两种（技术结论）**：RDB 是全量快照（fork 子进程写临时文件，恢复快、文件小，但两次快照之间的数据可能丢）；AOF（Append-Only File，追加式日志）记录每次写命令，按 `appendfsync` 策略控制刷盘，`everysec` 每秒 fsync、最多丢 1 秒数据。
...
- 配合 IO 多路复用（epoll），单线程就能支撑 10 万+ QPS。

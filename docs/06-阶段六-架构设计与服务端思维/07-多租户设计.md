...
**为什么「强制」可行**：应用是数据的唯一入口（生产环境没人绕过应用直连 DB），在 SQL 层统一注入 tenant_id，等于把「跨租户越权」从「每个接口记得校验」降维成「架构默认隔离」——人会忘，拦截器不会。
...
flowchart LR
    ...
    D --> E["MyBatis 租户拦截器<br/>自动补 WHERE tenant_id = 当前租户"]
    E --> F["数据库只返回<br/>本租户的行"]
...
- **连接数爆炸**（独立库模型）：每租户一个库，连接数随租户数线性增长。
  ...
  ```
- **索引要带租户前缀**：
  - **规则**：主查询都按 `tenant_id` 过滤，联合索引把 `tenant_id` 放最左（`(tenant_id, user_id, created_at)` 等）。
  - **原因**：B+ 树最左前缀原则——查询条件必须命中索引前缀列才走索引；`tenant_id` 不在最左 = 每次全表过滤再文件排序（filesort），「过滤租户 + 排序」双重慢。
...
**原理层**：
- 应用是数据的唯一入口，在 SQL 层统一注入（MyBatis-Plus 的 `TenantLineInnerInterceptor` 自动拼 `WHERE tenant_id = ?`），等于把「跨租户越权」从「每个接口记得校验」降维成「架构默认隔离」——人会忘，拦截器不会。
...
**工程层**：
- 缓存 key 统一模板 `user:profile:{tenantId}:{userId}`。
- 索引把 `tenant_id` 放最左（B+ 树最左前缀原则，否则全表过滤 + 文件排序）。
- 用 HikariCP 按租户池化并设上限。

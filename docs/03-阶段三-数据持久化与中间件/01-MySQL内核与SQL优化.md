...
| 可重复读（RR，默认） | 幻读被 Next-Key Lock 基本解决 | **事务内复用第一个 ReadView** |
| 串行化 | 无并发问题 | 读也加锁，性能差 |

- **快照读 vs 当前读**：普通 `SELECT` 是快照读（走 MVCC 不加锁）；`SELECT ... FOR UPDATE` / `UPDATE` / `DELETE` 是**当前读**（必须读最新版本并加锁）——这是「明明 RR 还会出现幻读」的原因，也是悲观锁 `FOR UPDATE` 的理论基础。
...
-- ② 隐式类型转换：order_no 是 VARCHAR，传数字 → 每行做类型转换 → 索引失效

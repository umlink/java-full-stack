...
```yaml
# ShardingSphere-JDBC 最小分库分表配置(按 user_id 取模分 4 库 8 表)
spring:
  shardingsphere:
    rules:
      sharding:
        # 分片键是 user_id: 决定这条数据落在哪个分库/分表
        sharding-column: user_id
        tables:
          order:
            actual-data-nodes: ds_${0..3}.order_${0..7}   # 4 库 × 8 表
            # 分库算法: user_id % 4 决定 ds_几; 分表算法: user_id % 8 决定 order_几
```

> 代码备注（逐行解释）：
> - `sharding-column: user_id`：分片键——按用户查是最高频场景，所以按 `user_id` 分片。
...
| 算法 | 原理 | 优点 | 坑 |
|-|-|-|-|
| **INLINE（取模）** | 分片键 % 分片数 | 简单、路由精准、好排查 | `%` 只能均匀；分片数一变全量重排——**换分片数 = 数据大迁移**（别改分片数，只扩备份） |
| **HASH_MOD（一致性哈希/哈希取模）** | 先 hash 再取模 | 分布更均匀（尤其分片键本身有规律时） | 同样受分片数约束 |

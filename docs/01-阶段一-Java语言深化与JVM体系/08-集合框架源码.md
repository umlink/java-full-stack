...
public class SimpleHashMap<V> {           // 教学版: 只保留 HashMap 的关键路径
    ...
    public void put(Object key, V value) {
        // 扰动函数: 高 16 位异或低 16 位 —— 把高位信息搅进低位, 定桶更均匀
        int hash = key.hashCode() ^ (key.hashCode() >>> 16); // >>> 是无符号右移: 连符号位一起右移补 0
        ...
        // 教学代码为可读性用「首插」(新节点挂桶头);
        // JDK 8 实际是「尾插」—— 头插在并发扩容时会成环 (JDK 7 经典事故),
        // 这是面试常追问的点, 下方「JDK 7 头插扩容成环」小故事专门展开
...
- **JDK 7 头插扩容成环**（**面试常追问，3 步小故事**）：
  - ① **同时触发扩容**：线程 A、B 都在搬同一桶的链表（原顺序 a→b→c）；
  - ② **头插逆序**：头插法 = 新节点插到桶头。A 搬完桶里变成 c→b→a；B 拿着自己记下的旧顺序再插一遍，插着插着 a.next 指向 b、b.next 又指回 a——**环成了**；
  - ③ **后果**：之后任何 get 落到这个桶都会在环里绕不出去——死循环、CPU 100%；
  - **JDK 8 改法**：改尾插不再成环，但并发 put 仍有数据覆盖——并发场景别赌运气，直接上 `ConcurrentHashMap`（见第 3 节）。

**3 步小故事一张图看懂**：

```mermaid
flowchart TD
    S["① 线程 A、B 同时触发扩容<br/>都在搬同一桶 a→b→c"]
    A["② A 头插搬完<br/>新桶 = c→b→a 逆序"]
    B["B 拿旧顺序 a→b→c 再插一遍<br/>头插到桶头"]
    R["插着插着 a.next 指向 b<br/>b.next 又指回 a —— 环成了"]
    D["③ 之后 get 落到这个桶<br/>在环里绕不出去 → 死循环 CPU 100%"]
    S --> A
    S --> B
    A --> R
    B --> R
...
- **扰动函数在算啥（一句话）**：把 32 位 `hashCode` 的**高 16 位与低 16 位异或**，让低位里混进高位的信息。
- **为什么必须混（不混会怎样）**：`hash & (n-1)` 定桶只取 hash 的**低 log₂n 位**（n=16 时只看低 4 位）。若不扰动，两个对象**高 16 位不同、低 4 位相同**，算出来就挤到同一个桶；扰动后「只看低 4 位」定桶，也能感知高位差异，分布更均匀。
- **生活类比**：只按门牌号「个位」给 1000 间房分楼道，10 层楼都挤同一条道；把「楼层号」也搅进个位，才散得开——扰动函数就是做这件事。
- **看数字走一遍**：

```java
int h = 0b1100_0101_1000_1111_0001_0010_0011_0100;  // 某 key 的 hashCode
//   高 16 位 = 1100_0101_1000_1111    低 16 位 = 0001_0010_0011_0100

int mixed = h ^ (h >>> 16);    // 扰动: 高 16 位 XOR 低 16 位 (逐位异或)
//    1100_0101_1000_1111
//  ^ 0001_0010_0011_0100
//  = 1101_0111_1011_1011      ← 高 16 位信息被搅进低 16 位

int bucket = mixed & 15;       // 定桶: n=16 时 hash & (n-1) 只看低 4 位 = 1011 → 桶 11
```

- **`hash & (n-1)` 就是取模（n=16 看数字）**：`hash & 15` 与 `hash % 16` 结果相同——15 的二进制是 `1111`，按位与正好**留下低 4 位**；位运算比除法快得多。
- **前提：n 必须是 2 的幂**：这条「位与 = 取模」只在 n 是 2 的幂时成立（这也是容量强制 2 的幂的原因，见下条）。
- **例外：n 不是 2 的幂（以 n=15 为例）**：n=15 时 `n-1 = 1110`，按位与会把 hash 的**最低位永远丢弃**——产生空洞，哈希退化成只能落到「偶数位」。
- **为什么容量必须是 2 的幂**（两个原因）：
  - **位运算顶替取模**：让 `hash & (n-1)` 等价于 `hash % n` 但快得多（n=16 时见上条「位与 = 取模」）；
  - **rehash 只看多出来的那一位**：扩容 16→32 时，旧桶 i 的元素新下标只有两个候选——hash 第 5 位是 0 就留原桶 i，是 1 就去「i+16」；元素只需「留原桶」或「原索引 + 旧容量」二选一，搬移成本极低。这就是思考题「自己推 resize」的钥匙。
- **为什么 JDK 8 引入红黑树（Red-Black Tree）**（**必答**）：一句话——哈希碰撞恶化时，桶内链表查找 O(n) → 树化后 O(log n)，把最坏情况拉回来。
- **红黑树是什么（白话）**：一棵**自平衡二叉搜索树**。二叉搜索树 = 「排序树」——每个节点最多两个孩子、左子树都比根小、右子树都比根大，查找像翻字典、每次能砍半；自平衡 = 每次增删后**自动收拾**，让树保持大致平衡、不歪。
- **靠哪两个动作维持平衡**：
  - **染色（红/黑）**：给每个节点标上红或黑，用颜色规则判断「这里要不要收拾」——像信号灯告诉你该停还是该走；
  - **旋转**：失衡时把局部父子关系转一下，像**扶正歪掉的书架**——只挪近处的几本就能摆正。这样树不会退化成「歪脖子链表」（歪脖子 = 失衡到极致，退化成单链表查找 O(n)）。
- **本质动机（防哈希碰撞攻击）**：攻击者能**恶意构造大量同桶 key**（都撞进同一个桶），把查询拖成 O(n)——这就是 DoS（拒绝服务）风险；树化后同桶最坏仍是 O(log n)，攻击手段失效。
...
public class ChmDemo {
    public static void main(String[] args) throws Exception {
        ...
        // 原子累加 —— 语义 ≈ Redis 的 INCR (原子自增命令);
        // Redis 是阶段三要学的缓存中间件, 这里先记住「并发自增不丢」这层意思即可
...
- **前端对照（为什么这套机制对你会陌生）**：NodeJS 单线程，两条 JS 不会「同时」改同一个集合，前端很少撞上这类问题；Java 多线程**真并行**，同一共享集合可能被并发改坏——fail-fast 就是把「用坏了却没察觉」变成「立刻报错」的通用机制（单线程用坏也拦得住，见下方示例）。
...
**答**：
- **标准结论**：JDK 8 的 HashMap 底层是「数组 + 链表 + 红黑树」；容量必须是 2 的幂，是为了能用位运算替代取模并让扩容成本极低。
- **原理层**：put 流程五步——
  - 对 key 做 `hashCode()` 后用扰动函数 `h ^ (h >>> 16)` 混合高低位；
  - 用 `hash & (n-1)` 定位桶（等价于取模但快得多）；
  - 桶空直接放，桶非空则遍历——key 相同覆盖 value，否则尾插进链表；
  - 链表长度 ≥ 8 且数组 ≥ 64 时树化为红黑树；
  - `size` 超过「容量 × 0.75」（负载因子）就扩容翻倍。
  - 容量必须 2 的幂有两个原因：
  - 一是 `hash & (n-1)` 能替代取模运算（位运算更快且分布均匀）；
  - 二是扩容时每个元素只需看 hash 多出来的那一位是 0 还是 1——留原桶或去「原索引 + 旧容量」，rehash 成本极低。
- **工程层**：预估规模时给初始容量（`new HashMap<>(1000)`），避免频繁扩容的整表拷贝；负载因子 0.75 是空间与冲突的平衡点，一般不动。

### Q2：HashMap 是线程安全的吗？并发下会出什么问题？ConcurrentHashMap 是怎么演进的？

**答**：
- **标准结论**：HashMap 不是线程安全的；并发共享的 Map 必须用 `ConcurrentHashMap`。
- **原理层**：
  - 为什么不安全——JDK 7 的 HashMap 并发扩容时头插法会形成循环链表，get 可能死循环（经典事故）；JDK 8 改为尾插不再成环，但并发 put 会数据覆盖、`size` 不准确；
  - 演进——JDK 7 是分段锁（Segment 数组，每段一把锁，默认 16 段，粒度粗、结构复杂）；JDK 8 改为「空桶 CAS 插入 + 非空桶 synchronized 锁桶头」，锁粒度细到单个桶，理论上并发度等于桶数；
  - `size()` 是多个计数单元（CounterCell）求和的**弱一致**估计值。
- **工程层**：
  - 先 get 后 put 这类两步操作在并发下依旧不安全，要用 `computeIfAbsent` / `merge` / `compute` 原子复合方法；
  - 遍历也不保证看到最新写入——需要强一致快照就得自己加锁。

### Q3：什么是 fail-fast？为什么遍历时修改集合会抛 ConcurrentModificationException？

**答**：
- **标准结论**：fail-fast（快速失败）是 Java 集合的**错误检测机制**，不是并发保护——结构被改就立刻抛异常，目的是「尽早暴露 bug」。
- **原理层**：
  - `ArrayList` / `HashMap` 内部维护 `modCount`（结构性修改计数），迭代器创建时记住 `expectedModCount`；
  - 每次 `next()` 都校验两者是否相等——不等就抛 `ConcurrentModificationException`；
  - 设计动机：迭代时修改结构，语义本就未定义，与其悄悄给出错误结果，不如立刻崩溃让你发现。注意**单线程也会触发**（遍历中 `list.remove()`），所以它是通用防御。
- **工程层**：正确做法三选一——迭代器自己的 `remove()`（会同步更新 `expectedModCount`）、`removeIf`、收集到新集合再删；并发场景读多写少用 `CopyOnWriteArrayList`——迭代器持旧数组快照所以不抛异常，但每次写都全量复制数组，写频繁代价极高。

### Q4：ConcurrentHashMap 的锁粒度是什么？为什么跳表（ConcurrentSkipListMap）比红黑树更适合并发有序容器？

**答**：
- **标准结论**：JDK 8 的 ConcurrentHashMap 锁粒度是**单个桶**；跳表因写操作只改局部指针、能逐节点 CAS 无锁化，比红黑树更适合做并发有序容器。
- **原理层**：
  - 锁粒度——空桶插入用 CAS 无锁完成，非空桶用 synchronized 锁桶头节点，互不干扰的桶可完全并行，相比 JDK 7 的分段锁（16 个段）并发度大幅提升；
  - 跳表（Skip List）是「底层全量有序链表 + 上层抽点建索引」的多层结构，查找平均 O(log n)；
  - 为何比红黑树适合并发——红黑树的插入/删除要旋转并调整全树结构，涉及多节点指针变更，并发下必须拿全局锁；而跳表的写操作只影响目标节点的邻居指针，可以逐节点用 CAS 原子更新，天然无锁化——所以 Java 的有序并发 Map 选了 `ConcurrentSkipListMap`。
- **工程层**：排行榜、延迟任务（按时间戳 `headMap` 取到期任务）这类「有序 + 并发」需求。

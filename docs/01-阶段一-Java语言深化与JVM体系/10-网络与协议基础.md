...
| HTTP/3 | **QUIC（Quick UDP Internet Connections，基于 UDP）** | TCP 层队头阻塞 + 握手往返（0/1-RTT） |
...
- **TLS（Transport Layer Security，传输层安全协议）**：跑在 TCP 与 HTTP 之间的加密层——JVM 信任库（cacerts）决定「信哪些 CA」；**SNI（Server Name Indication，服务器名称指示）**：一个 IP 上挂多张证书时，客户端在握手时用 SNI 字段标明要哪张——这是「单 IP 多域名共用 443 端口」的前提；**mTLS**（双向认证）：双方都要校验对方证书，服务间零信任的基石，阶段六第 5 讲。

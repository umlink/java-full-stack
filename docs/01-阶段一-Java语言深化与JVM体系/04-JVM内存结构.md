...
- **排查 OOM 三步**：看异常类型（`Java heap space` 堆 / `StackOverflowError` 栈 / `Metaspace` 元空间）→ 用 `jmap`/`jstat`（JDK 自带的内存观测工具）看内存构成 → 定位谁在涨（通常是某个集合/Cache 无限增长）。
...
**生活版**：公司有四样东西各管一摊——**大仓库**进什么货都先进仓，定期有人打扫，把没人要的货清出去；**员工工位**每接一个任务就压一叠单据，单据叠太高会塌；**档案室**存着每个岗位的「岗位说明书」；你手里还有本**书签**，记着看到第几页。四样东西各司其职。

**换成 Java**：大仓库就是**堆**——进货（`new`）都先进仓，定期打扫就是 GC（把没人要的货清出去）；员工工位就是**虚拟机栈**——每接一个任务（方法调用）压一叠单据（栈帧），单据叠太高会塌（StackOverflowError）；档案室就是**元空间**——存每个类的「岗位说明书」（类定义信息）；书签就是**程序计数器**——记着当前读到哪一行字节码。**四块各司其职，哪块出问题，抛的异常都不一样**——这就是「先问是哪块满了」的排查心智。

| 区域 | 存什么 | 典型异常 |
|-|-|-|
| 虚拟机栈 | 每个方法调用压入一个**栈帧（Stack Frame）**（栈帧的构成见下注） | `StackOverflowError`（深递归） |
...
> 注：**栈帧（Stack Frame）** = 一次方法调用压进栈的「一叠单据」，四部分各记一句——**局部变量表**（方法内的局部变量、参数放这）、**返回地址**（方法结束后回到哪一行继续执行）、**操作数栈**（进阶名词：方法内部的临时计算区，算表达式的中间结果先放这）、**动态链接**（进阶名词：运行时把符号引用解析成具体实现）。操作数栈与动态链接属进阶内容，先跳过不影响理解。
...
- **内存泄漏的通用指纹**：集合 / 缓存 / 监听器**只进不出**——只 add 不 remove，对象一直被强引用拽着，GC 清不掉。
- **定位工具链三步**（jmap → MAT → 支配树）：
    1. **dump 堆**：用 `jmap -dump` 把堆上对象的引用关系**快照成文件**（dump = 堆快照，把内存当场拍一张照片存下来分析）；
    2. **MAT 打开 dump**：MAT（Eclipse Memory Analyzer，堆转储分析工具）把快照读成可分析的视图；
    3. **看支配树**：切到 **支配树（Dominator Tree）** 视图，找「谁占着最大一块引用链」——顺着最大引用链就能摸到元凶。

```mermaid
flowchart TB
    A[堆 OOM] --> B[jmap -dump<br/>留一份 dump 堆快照]
    B --> C[MAT 打开 dump]
    C --> D[支配树视图<br/>找「最大引用链」元凶]
    D --> E[修：集合只进不出<br/>缓存设上限等]
```
...
下面这段**全是注释，不能运行**，是「元空间怎么被动态类撑满」的示意小剧场——帮你在脑内建立因果：
...
**答**：
**标准结论**：运行时数据区按「线程私有 / 线程共享」分两类——**线程私有**：程序计数器、虚拟机栈、本地方法栈；**线程共享**：堆、元空间（JDK 8 起替代永久代 / 方法区）。除了程序计数器，其余区域异常超限都可能抛错。

**原理层**：
- 各自职责：堆存对象实例与数组（GC 主战场）；虚拟机栈每个方法调用压一个栈帧（局部变量表、操作数栈、动态链接、返回地址）；元空间存类元信息；程序计数器记录当前线程执行到第几条字节码。
- 异常对应：深递归 → `StackOverflowError`；堆满 → `OutOfMemoryError: Java heap space`；元空间满 → `OutOfMemoryError: Metaspace`；程序计数器是唯一不会 OOM 的区域。

**工程层**（面试加分点）：
- 虚拟机栈也可能 OOM：栈深度超限默认抛 `StackOverflowError`，但**栈大小申请失败**会抛 OOM。
- 元空间用本地内存所以「默认无上限」——Metaspace OOM 几乎必然意味着**动态类生成失控**（反射 / CGLIB 缓存泄漏），不是参数给小了。

### Q2：双亲委派模型是什么？为什么这么设计？如何打破？
**答**：
**标准结论**：双亲委派是类加载器的协作模型——一个类要加载时，**先逐级委托父加载器，父级加载不了才由自己加载**。

**原理层**：
- 系统三层：Bootstrap（JDK 核心库，C++ 实现，Java 里表现为 null）→ Platform（JDK 9 起，JDK 8 时代是 Extension）→ Application（classpath）。
- 设计目的有二：① **安全**——核心类只能由 Bootstrap 加载，业务代码无法伪造 `java.lang.String`；② **一致性**——同一个类只加载一次（类的唯一性由「全限定名 + 类加载器」共同决定）。

**工程层**（打破的经典场景）：
- **SPI**：JDBC Driver 由 Bootstrap 启动、但实现类在 classpath，用**线程上下文类加载器反向加载**。
- **Tomcat 容器**：每个 Web 应用独立加载类，实现**类隔离与热部署**。
- **OSGi**：**平级委派**。
- **自定义加载器**：只需继承 `ClassLoader` 覆写 `findClass`——默认遵守双亲委派；**只有覆写 `loadClass` 才是真正打破**。

### Q3：永久代和元空间有什么区别？为什么用元空间替代永久代？
**答**：
**标准结论**：永久代（PermGen）是 JDK 8 之前的方法区实现，元空间（Metaspace）是 JDK 8 起的替代——本质差异是「类元信息放哪」。

**原理层**（三大区别）：
- ① **内存位置**：永久代在 JVM 管理的**堆内**（受 `-XX:MaxPermSize` 限制）；元空间用**本地内存**（默认只受 OS 内存约束）。
- ② **管理方式**：永久代大小固定、易「调不准就 OOM」；元空间按需扩展。
- ③ **内容变化**：字符串常量池从永久代移到堆上（JDK 7 起）；类元信息进元空间。

**工程层**（为什么换 + 面试注意）：
- 为什么换：永久代的「固定上限」与动态类加载（框架代理、热部署、Groovy 等）的矛盾无法调和，PermGen OOM 频繁成为线上事故；元空间把「类元信息的内存预算」交还给 OS，从根上消灭了这类调参事故。
- 面试注意：Metaspace OOM 现在意味着**动态类生成失控**（如每次请求生成新代理类且缓存失效），而不是「默认给小了」。

### Q4：栈溢出和堆溢出怎么区分？堆 OOM 如何定位内存泄漏？
**答**：
**标准结论**：按**异常类型**区分——栈溢出是 `StackOverflowError`（递归无出口或过深，栈帧压满栈）；堆溢出是 `OutOfMemoryError: Java heap space`（对象只增不减把堆填满）。

**原理层**（定位方法分治）：
- 栈溢出定位：先看堆栈打印找递归点；生产解法不是加大 `-Xss`（只是死得慢一点），而是**改迭代 / 加深度上限**。
- 堆溢出三步走：① 看异常类型确认是堆；② 用 `jmap -dump` 留 dump，或靠生产基线参数 `-XX:+HeapDumpOnOutOfMemoryError` 自动留 dump；③ 用 MAT 看支配树（Dominator Tree）——找「谁占着最大一块引用链」。
- 内存泄漏的通用指纹：集合 / 缓存 / 监听器只进不出——List 只 put 不 remove、线程池队列无界、ThreadLocal 不 remove（线程池里 value 永远可达）都是经典元凶。

**工程层**（区分「泄漏 vs 流量」）：
- **泄漏**：老年代单调涨、Full GC 后不回落。
- **流量**：年轻代高频 GC、但能回收干净。

### Q5：为什么服务刚启动慢、跑一会儿变快？
**答**：
**标准结论**：这是 **JIT 分层编译 + 缓存填充**的「预热」现象——不是玄学，也不是 bug。

**原理层**：
- Java 先解释执行字节码，运行中 JVM 统计热点方法，达到阈值后**逐级升级编译**：C1（Client Compiler）快速产出机器码；C2（Server Compiler）基于 profiling 做深度优化（方法内联、逃逸分析、锁消除）。
- 刚启动：大量代码还在解释执行、各类缓存（CPU 缓存、连接池、框架元数据）也未填充 → RT 偏高；跑一段后：热点方法被编译成机器码 → 速度上来。

**工程层**（实战落点）：
- 压测要跑过预热期再采数，否则冷启动数据严重误导容量结论。
- 生产用**启动预热流量 + readiness 探针延迟放行**（K8s startupProbe），让服务完成预热再接入流量。
- 别把「刚启动慢」当 bug 排查，先确认是不是预热窗口。
- 延伸：这也是「有些优化在 benchmark 里有效、生产无效」的底层原因之一。

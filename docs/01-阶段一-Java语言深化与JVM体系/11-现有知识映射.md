...
try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {   // JEP 505(预览): 结构化并发(JEP = JDK 特性提案编号, 505 是它的提案号)
...
- **把 Promise 的微任务语义套到 CompletableFuture**：JS 的 `.then` 回调进微任务队列「几乎立即」执行；Java 的 `thenApply` 在**哪个线程**执行取决于完成时机（完成前注册 → 执行器线程；完成后注册 → 调用方线程）——依赖「下一个 tick」的代码会偶发错线程。会坏的具体场景：任务完成后在 main 线程注册回调，回调却跑在调用方线程，读线程本地变量拿到 null 或别人的值：

```java
var f = CompletableFuture.supplyAsync(() -> "done", pool);
Thread.sleep(100);                       // 确保 f 已完成
String v = f.thenApply(s -> {            // 此时注册 → 回调跑在 main 线程
...
**答**：NodeJS 是单线程事件循环：所有 JS 代码跑在一个线程上，IO 通过 libuv 交给底层（Linux 上是 epoll），回调进事件队列；好处是「单线程无锁」，代价是 CPU 密集任务会卡死整个进程、同步阻塞写法是禁区。Java 默认是多线程 + 显式并发控制：共享可变状态要靠锁 / 原子类，同步写法是常态。虚拟线程出现后两者在「IO 密集」场景殊途同归——都是「等的时候不占线程」：Node 用非阻塞回调，虚拟线程用「阻塞但便宜」。面试表达的关键是别把「回调别阻塞事件循环」的直觉硬搬到 Java——虚拟线程时代，同步写法反而更简单、更易读、更接近业务直觉。

### Q2：goroutine 和虚拟线程有什么异同？channel 在 Java 里的对应物是什么？

**答**：相同点：都是轻量级执行单元，创建成本远低于 OS 线程，阻塞时不占系统线程，都靠调度器在少量载体线程上复用（Go 的 M:N 调度、JVM 的 carrier 线程）。差异：goroutine 配 channel 的 CSP（通信顺序进程）模型——「不共享内存、靠通信」，channel 既是通信也是同步；Java 虚拟线程共享堆内存，通信靠共享对象 + 锁/原子类，没有 channel 这样的语言级原语。Java 的对应答案：任务编排用 CompletableFuture（Promise 组合），「生产者-消费者」队列场景用 `BlockingQueue`，任务组「成败与共」用结构化并发（StructuredTaskScope）。工程差异：goroutine 的成本模型让「随手开协程」成为习惯，Java 侧也要用虚拟线程才能获得同等成本感——用平台线程硬扛就是灾难。

### Q3：从 Node 团队迁到 Java，最容易踩的并发坑有哪些？

**答**：三个典型：① 把 Promise 的微任务语义套到 CompletableFuture——JS 的 `.then` 回调进微任务队列几乎立即执行，Java 的 `thenApply` 在哪个线程跑取决于完成时机（完成前注册走执行器线程、完成后注册走调用方线程），依赖「下一个 tick」的代码会偶发错线程；② 用平台线程的复用心智管理虚拟线程——虚拟线程廉价、用完即弃，池化是反模式，批量任务用 `newVirtualThreadPerTaskExecutor()`；③ 忽视共享状态同步——Node 单线程无竞争，Java 多线程下 `HashMap`、`SimpleDateFormat` 这类非线程安全类会被并发用坏，共享集合换 `ConcurrentHashMap` / `CopyOnWriteArrayList`，可变对象访问要同步。代码评审 checklist：所有异步任务是否显式指定执行器、`Future.get()` 是否带超时、共享对象是否线程安全、线程池是否显式七参数。

...
> **关键差异**：JS 的 Promise 由事件循环免费调度，Java 的必须指定线程池（否则走公共 ForkJoinPool——全 JVM 只有这一份的「公共会议室 / 共享打印机」：IO 任务一占一小时，CPU 任务永远订不到——IO 阻塞会饿死它）；JS 有 `async/await` 语法糖，Java 靠链式 + `join()`。
...
```javascript
// Node: Worker = 独立 isolate(V8 的独立内存空间实例——Worker 线程各有一个), 靠消息传递通信(内存不共享)
...
> （背景铺垫：虚拟线程本身不占平台线程，而是跑在少量**载体线程（carrier 线程）**上——像多个轻任务轮流占用少数几个工位，任务阻塞时把工位让出来给别人用；这就是「虚拟线程很便宜、随便开」的来源，面试答案里也常考到。）
...
// JEP = JDK 特性提案编号(JEP 全称: JDK Enhancement Proposal), 505 是它的提案号
try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {   // 结构化并发(JEP 505, 预览特性)
...
> **关键差异**：goroutine 配 channel 的 CSP（Communicating Sequential Processes，通信顺序进程）模型 Java 没有直接对应物。翻成白话：Go 的思路是「东西不直接塞你手里，放上传送带（channel）递给你」；Java 的思路是「大家围着一块共享黑板读写，用锁维持秩序」。两种哲学的对偶图：
>
> ```mermaid
> flowchart LR
>     subgraph Go侧[Go：不共享内存，靠通信]
>         G1[goroutine A] -->|数据放上传送带| C[(channel 传送带)] -->|取走数据| G2[goroutine B]
>     end
>     subgraph Java侧[Java：共享内存，靠锁维持秩序]
>         J1[线程 A 写黑板] --> B[(共享黑板<br/>共享内存)]
>         J2[线程 B 读黑板] --> B
>         L1[锁 / 原子类<br/>维持秩序] -.保护.-> B
>     end
> ```
>
> Java 的对应答案：**结构化并发（Structured Concurrency）**（任务组作为一个单元成败与共）+ 阻塞队列。

> ⏸️ **短期可以不学**：结构化并发（`StructuredTaskScope`，JEP 505）目前仍是**预览 API**——业务代码主流还是线程池 + CompletableFuture，别在生产依赖预览特性。**何时回来学**：它转正式版且公司技术栈跟进，或要写「任务组成败与共」的编排代码时。**面试最低要求**：能说「Java 没有 Go 的 channel，并发编排的答案是 CompletableFuture + 阻塞队列；结构化并发是更安全的任务组写法、目前仍是预览特性」即可。
...
- **把 Promise 的微任务语义套到 CompletableFuture**：JS 的回调调度和 Java 完全两套，硬搬会在三个层面错位：
  - **微任务语义差异**：JS 的 `.then` 回调进**微任务队列**，「几乎立即」执行；Java 的 `thenApply` 回调由线程池调度，没有「下一 tick 必达」的保证。
  - **注册时机决定执行线程**：`thenApply` 在**哪个线程**执行取决于完成时机——完成前注册 → 执行器（线程池）线程执行；完成后注册 → 调用方线程就地执行。
  - **依赖「下一个 tick」会偶发错线程**：把「回调一定马上执行」当假设写进代码，时机一错线程就错。
  - **ThreadLocal 随线程失效**：回调换了线程，线程本地值就读不到了——会拿 null 或别人的值（类比：ThreadLocal 像每个线程专用的「储物格」，换线程等于换个人去开别人的格子）。

> 会坏的具体场景 + 示意代码（看懂思路即可，不必真跑）：任务完成后在 main 线程注册回调，回调却跑在调用方（main）线程。`pool` 代指「示意线程池」，`ThreadLocalHolder` 是「示意工具类」——它内部才真正持有 ThreadLocal，外面这么写只是贴近常见封装。

```java
var f = CompletableFuture.supplyAsync(() -> "done", pool);  // 任务交给示意线程池 pool
Thread.sleep(100);                       // 确保 f 已完成
String v = f.thenApply(s -> {            // 完成后才注册 → 回调跑在调用方(main)线程
    return ThreadLocalHolder.get();      // 读 ThreadLocal/ScopedValue → null 或别人的值
}).join();                               // 而非预期的 pool 线程
```

> 一句结论：依赖「回调一定在我注册的地方执行」= 把 JS 微任务语义硬搬过来，偶发错线程。
- **把 goroutine 成本模型套到平台线程**：
  - **平台线程很贵**：`new Thread(...)` 每个都是 OS 级重资源（约 1MB 栈）。
  - **要 goroutine 的「便宜随手开」成本感**：必须用虚拟线程。
- **把 EventEmitter 的「同步调用回调」套到 Spring 事件**：
  - **默认是同步的**：`@EventListener` 在发布者线程执行、同一事务内。
  - **想异步**：加 `@Async` + `@EnableAsync`。
  - **只想「事务提交后」触发**：用 `@TransactionalEventListener(AFTER_COMMIT)`——跨了事务边界后行为完全不同。
...
### Q1：你之前写 NodeJS，事件循环和 Java 的并发模型有什么本质差异？

**答**：**标准结论**：NodeJS 是「单线程事件循环 + 非阻塞回调」，Java 是「多线程 + 显式并发控制」；虚拟线程让两者在「IO 密集」场景殊途同归——都是「等的时候不占线程」。
**原理层**：
- **NodeJS**：单线程事件循环——所有 JS 代码跑在一个线程上，IO 通过 libuv 交给底层（Linux 上是 epoll），回调进事件队列。好处是「单线程无锁」。
- **它的代价**：CPU 密集任务会卡死整个进程、同步阻塞写法是禁区。
- **Java**：默认多线程 + 显式并发控制——共享可变状态要靠锁 / 原子类，同步写法是常态。
- **汇合点**：Node 用非阻塞回调「等」，虚拟线程用「阻塞但便宜」等——都不占线程。
**工程层**：面试表达的关键——别把「回调别阻塞事件循环」的直觉硬搬到 Java：虚拟线程时代，同步写法反而更简单、更易读、更接近业务直觉。

### Q2：goroutine 和虚拟线程有什么异同？channel 在 Java 里的对应物是什么？

**答**：**标准结论**：两者同为「轻量执行单元、阻塞不占系统线程、靠调度器复用载体线程」；差异在通信哲学——Go 用 channel「不共享内存、靠通信」，Java 共享堆 + 锁/原子类、没有语言级 channel。channel 的 Java 对应物是 CompletableFuture + BlockingQueue + 结构化并发。
**原理层**：
- **相同点**：都是轻量级执行单元，创建成本远低于 OS 线程；阻塞时不占系统线程；都由调度器在少量载体线程上复用（Go 的 M:N 调度、JVM 的 carrier 线程）。
- **差异点**：goroutine 配 channel 的 CSP（通信顺序进程）模型——「不共享内存、靠通信」，channel 既是通信也是同步；Java 虚拟线程共享堆内存，通信靠共享对象 + 锁/原子类，没有 channel 这样的语言级原语。
- **channel 的对应答案**：任务编排用 CompletableFuture（Promise 组合）；「生产者-消费者」队列场景用 `BlockingQueue`；任务组「成败与共」用结构化并发（StructuredTaskScope）。
**工程层**：goroutine 的成本模型让「随手开协程」成为习惯；Java 侧也要用虚拟线程才能获得同等成本感——用平台线程硬扛就是灾难。

### Q3：从 Node 团队迁到 Java，最容易踩的并发坑有哪些？

**答**：**标准结论**：三个典型坑——微任务语义错位、用平台线程心智管虚拟线程、忽视共享状态同步；再配一套代码评审 checklist 兜底。
**原理层**：
- **坑① 微任务语义错位**：把 Promise 的微任务语义套到 CompletableFuture——JS 的 `.then` 回调进微任务队列几乎立即执行；Java 的 `thenApply` 在哪个线程跑取决于完成时机（完成前注册走执行器线程、完成后注册走调用方线程），依赖「下一个 tick」的代码会偶发错线程。
- **坑② 用平台线程心智管虚拟线程**：虚拟线程廉价、用完即弃，池化是反模式，批量任务用 `newVirtualThreadPerTaskExecutor()`。
- **坑③ 忽视共享状态同步**：Node 单线程无竞争，Java 多线程下 `HashMap`、`SimpleDateFormat` 这类非线程安全类会被并发用坏——共享集合换 `ConcurrentHashMap` / `CopyOnWriteArrayList`，可变对象访问要同步。
**工程层**（代码评审 checklist 落地）：所有异步任务是否显式指定执行器、`Future.get()` 是否带超时、共享对象是否线程安全、线程池是否显式七参数。

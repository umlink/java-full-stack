...
- **AQS（AbstractQueuedSynchronizer）**：`ReentrantLock` / `Semaphore` / `CountDownLatch` 全构建在它之上。核心就两样：一个 `volatile int state`（语义由子类定义——锁的占用 / 许可数 / 倒计数）+ 一条 CLH 等待队列（获取失败的线程挂起排队）。

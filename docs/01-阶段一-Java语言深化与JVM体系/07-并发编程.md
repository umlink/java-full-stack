...
- **AQS（AbstractQueuedSynchronizer）**：`ReentrantLock` / `Semaphore` / `CountDownLatch` 全构建在它之上。核心就两样：一个 `volatile int state`（语义由子类定义——锁的占用 / 许可数 / 倒计数）+ 一条 CLH（Craig–Landin–Hagersten）等待队列——获取失败的线程不自旋空转，而是**挂起（park）排队**，释放时唤醒队头；把「等锁」从烧 CPU 变成让出 CPU，这是 AQS 能扛高并发争抢的关键设计。

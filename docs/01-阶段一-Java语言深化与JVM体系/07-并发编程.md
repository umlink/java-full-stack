...
> - `new ArrayBlockingQueue<>(10)`：**有界队列**——队列满才扩到最大线程，避免无限堆积 OOM（什么是「有界」见正文第 3 节）。
...
**先把 workQueue 讲透：什么叫「有界队列」**

- **阻塞队列（BlockingQueue）**：线程安全的队列，且「满了放不进会让线程等待、空了取也会等待」——线程池的 workQueue 参数必须是这种类型（任务放进队列、工作线程从队列取，两头的等待都由它管）。
- **`new ArrayBlockingQueue<>(10)` 拆开读**：`ArrayBlockingQueue` 是 JUC 提供的「底层用**定长数组**实现的阻塞队列」，`10` 就是**队列容量**——最多缓存 10 个排队中的任务，先进先出；数组在创建时一次性分配，内存紧凑。
- **「有界」vs「无界」——差别就是有没有这行保护**：

| | 有界队列（推荐） | 无界队列（坑） |
|-|-|-|
| 写法 | `new ArrayBlockingQueue<>(200)` 或 `new LinkedBlockingQueue<>(100)` | `new LinkedBlockingQueue()`（不传容量） |
| 容量 | 你指定的上限 | 默认 `Integer.MAX_VALUE` ≈ 21 亿——约等于没有 |
| 队列满后 | 扩线程到 max → 仍满 → **触发拒绝策略**——压力有出口 | 永远装得下，永不触发拒绝 |
| 结局 | 过载被拒绝策略拦下（可观测、可处理） | 任务无限堆积，直到 **OOM** |

> 注意：坑不在 `LinkedBlockingQueue` 这个类——它传了容量同样是有界的（`LinkedBlockingQueue` 底层是链表，每个元素一个节点对象，适合容量较大或需动态的场景）；坑在「**没设容量**」。`Executors.newFixedThreadPool` 内部正是 `new LinkedBlockingQueue<>()` 不传容量，这就是 L64 红线「无界队列会 OOM」的完整解释。

**生活版（把排号凳讲透）**：餐厅门口的排号凳是**有限的**——坐满了才招临时工（扩到 max）、连凳子都没了才立牌拒客（拒绝策略），压力总能走到「被处理或被拒绝」的出口。要是排号凳**无限长**（无界队列）：谁都不会被拒绝、队伍越排越长，最后把整本登记簿撑爆（OOM）——短期看「没人被拒绝」很友好，实际只是把崩溃推迟到更难排查的时刻，而且**连一条「满了」的日志都没有**。

**换成 Java**：队列是压力的「蓄水池」——有界才有「满」的概念，「满」才能依次触发扩容与拒绝这些保护动作；无界队列等于拆掉了水位警报器。所以红线要求一律写 `new ArrayBlockingQueue<>(具体容量)`（容量按业务峰值估，参考本节参数估算）。
...
```mermaid
flowchart TD
    A["提交一个新任务<br/>（比如一个 HTTP 请求进来）"] --> B{"核心线程<br/>都忙吗？"}
    B -->|"有空闲核心线程"| C["复用/新建核心线程<br/>直接执行任务<br/>（正式工接活）"]
    B -->|"核心线程全忙"| D["任务放进有界队列排队<br/>（门口排号凳等位）"]
    D --> E{"队列满了吗？"}
    E -->|"没满"| F["任务在队列里等<br/>有线程空了再来取"]
    E -->|"满了"| G{"线程数到<br/>maximumPoolSize 了吗？"}
    G -->|"还没到上限"| H["扩到最大线程<br/>新建救急线程执行<br/>（招临时工）"]
    G -->|"已到上限"| I["触发拒绝策略<br/>（客满立牌拒客：<br/>Abort 抛异常／CallerRuns 让提交方自己跑）"]
    F --> J["有线程空闲时<br/>取出任务执行"]
    C --> J
    H --> J
```
...
**把散落对照汇总成一张前端对照表**（CompletableFuture 的「JS 心智锚点」，代码示例里已用上大半）：

| `CompletableFuture` | ≈ 前端对照 | 白话用途 |
|-|-|-|
| `supplyAsync(fn, pool)` | ≈ `new Promise` | 发起一个异步任务，放进指定执行器去跑 |
| `allOf(a, b, c)` | ≈ `Promise.all` | 等几路全部完成再继续 |
| `anyOf(a, b)` | ≈ `Promise.race` | 任一先完成就继续 |
| `thenApply(fn)` | ≈ `.then(v => …)` | 上一个结果出来，同步转换一下 |
| `thenCompose(fn)` | ≈ 返回新 Promise 的自动拍平（前端 flatMap） | 串联第二个异步任务，等它完成 |
| `thenCombine(b, fn)` | ≈ 手写 `Promise.all` 后再合并 | 两路结果都到了，合成一个 |
| `exceptionally(fn)` | ≈ `.catch` | 任一环节抛错，走降级兜底 |
| `orTimeout(...)` | JS 需自写 `Promise.race` + 定时器 | 超时抛异常，防永久挂起 |
...
**答**
- **标准结论**：`ThreadPoolExecutor` 七参数 = corePoolSize（核心线程）、maximumPoolSize（最大线程）、keepAliveTime + TimeUnit（非核心线程空闲存活时间）、workQueue（任务队列）、threadFactory（线程工厂）、RejectedExecutionHandler（拒绝策略）；执行次序是「先核心 → 再队列 → 后扩 max → 最后拒绝」。
- **原理层**：
  - 执行流程：先占满核心线程 → 任务进队列 → 队列满才扩到最大线程 → 队列和线程都满才触发拒绝策略。
  - 核心线程数估算：CPU 密集 ≈ 核数 + 1（避免线程切换浪费 CPU）；IO 密集 ≈ 核数 ×（1 + 等待时间/计算时间）——起点只是估算，最终靠压测修正。
- **工程层**（红线 + 真实项目里的坑）：
  - 不用 `Executors.newFixedThreadPool` / `newCachedThreadPool`（无界队列 OOM、最大线程数失控），必须显式 `new ThreadPoolExecutor`，配**有界队列 + 命名线程工厂 + 明确拒绝策略**。
  - 拒绝策略选型：`CallerRuns` 适合愿意接受反压的场景（调用方线程执行任务，把压力反弹给上游）；`Abort` 是默认（抛异常要接住）；`Discard` 静默丢任务生产慎用。
  - 常见误区①：把核心线程数设成「并发请求数」——请求是瞬时的，稳态在线数才是核心线程该覆盖的。
  - 常见误区②：`Future.get()` 不带超时——下游一卡，整个池被拖死。

### Q2：volatile 和 synchronized 有什么区别？volatile 能保证线程安全吗？

**答**
- **标准结论**：不能——volatile 保证**可见性**与**有序性**，**不保证原子性**，单独使用不线程安全；synchronized 三者都保证。
- **原理层**：
  - volatile 的可见性：写 volatile 变量立即刷主存，其他线程读时失效缓存重新加载。
  - volatile 的有序性：禁止指令重排，防止「对象已发布但构造未完成」这类问题。
  - volatile 不保证原子性：`volatile int i; i++` 是读-改-写三步，两个线程同时执行依旧丢更新。
  - synchronized 三者都保证：进入/退出监视器（Monitor）建立 happens-before（先行发生——前一个操作的写保证对后一个操作的读可见）关系（可见性）；并发的读-改-写被互斥串行化（原子性）；同步块内外的重排被限制（有序性）。
- **工程层**（怎么选、怎么用对）：
  - 场景选型：「多线程计数器」用 `AtomicInteger`（CAS）；「单标志位可见」用 volatile（如状态开关、双重检查锁里的 instance 字段）；「一段复合操作要互斥」用 synchronized / `ReentrantLock`。
  - 工程误区：「加了 volatile 就安全」——它管的是「看得见」和「不乱序」，管不了「两步操作之间别人插进来」。

### Q3：虚拟线程和平台线程有什么区别？为什么说它不能替代 CPU 密集的并行计算？

**答**
- **标准结论**：判断一句话——**在等（IO）用虚拟线程，在算（CPU）用并行计算**。虚拟线程只是把阻塞变廉价、并没有增加 CPU 核数，所以替代不了 CPU 密集的并行计算。
- **原理层**：
  - 平台线程是 1:1 映射到 OS 线程的重资源（约 1MB 栈 + 内核调度实体）；虚拟线程（Project Loom，JDK 21 正式版）是 JVM 调度的轻量线程，阻塞时 JVM 会把载体线程释放去跑别的虚拟线程——因此「一请求一线程」的同步写法也能扛海量 IO 并发。
  - 为什么替代不了 CPU 密集：CPU 密集任务不需要等待，虚拟线程的挂载/卸载调度反而变成纯开销——此时该用并行流 / 固定大小线程池（核数是硬上限）。
- **工程层**：
  - 虚拟线程不要池化（用完即弃）。
  - `synchronized` 块内阻塞在 JDK 24 前会 pin 住载体线程——热点路径用 `ReentrantLock` 规避。
  - 批量任务用 `Executors.newVirtualThreadPerTaskExecutor()` 统一生命周期。

### Q4：CAS 的原理是什么？它有什么问题，怎么解决？

**答**
- **标准结论**：CAS（Compare-And-Swap）是 CPU 提供的原子指令——「我认为内存值是 A，如果是就改成 B，否则失败并返回当前值」。它是 Java 无锁编程的地基，`AtomicInteger` 的自增就建立在它之上。
- **原理层**：
  - 底层实现：`AtomicInteger` 用 `Unsafe.compareAndSetInt` 循环重试——拿旧值 → 算新值 → CAS 替换，失败说明有并发修改，重来。
  - 与锁的取舍：CAS 无上下文切换、无死锁，适合轻量短临界区；但临界区长、竞争激烈时，锁（让出 CPU + 排队）反而吞吐更高。
- **工程层**（三个经典问题与解法）：
  - ① ABA（值 A→B→A，CAS 以为没人动过）：用 `AtomicStampedReference`（带版本号）解决。
  - ② 自旋开销（竞争激烈时空转烧 CPU）：`LongAdder` 把单热点拆成多个 Cell 分散竞争。
  - ③ 只能保证单变量原子：多变量要加锁，或装进一个对象用 `AtomicReference`。
  - 生产分工一句话：「计数用 Atomic / LongAdder，复合操作用锁」。

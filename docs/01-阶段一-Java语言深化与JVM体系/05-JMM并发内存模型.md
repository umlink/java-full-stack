...
**底层怎么做到**：volatile 读写会在周围插入**内存屏障（Memory Barrier）**——volatile 写前的普通写不能「滑」到它之后（必须先落对位置再发布标志），volatile 读后的普通读不能「滑」到它之前；屏障再配合 CPU 的缓存一致性协议（如 MESI——协调各 CPU 核心缓存的四状态协议，保证同一个变量的多份缓存副本不打架），才实现「改了就看得见」。注意它**不做第三件事——不保证原子性**：`volatile i++` 的「读-改-写」三步依旧会被交错，照样丢更新。

...
| EventEmitter | ApplicationEvent | Spring 事件默认同步执行，还能挂在事务提交时机上（`@TransactionalEventListener`：事务回滚就不发——EventEmitter 没有这个维度）；详见面试题 Q4 |
...
- **先映射、再记差异、最后练手**：这是这套文档反复强调的对比学习法——新概念先对应熟悉的 NestJS 概念，再补差异。类比「**方言同义词典**」：就像广东话母语者学普通话，大部分词同源、看着就懂（映射表），少数词同形不同义、必须单独记（「关键差异」列），还有极少数词广东话里根本没有（Spring 独有的 AOP 代理层）。
- **Spring 隐式 vs NestJS 显式**：Spring 组件扫描是隐式的（同包自动扫），NestJS 模块关系是显式声明——想象「**自动门 vs 刷卡进门**」：Spring 的 `@ComponentScan` 是自动门，走近就开、全靠约定（同包及子包即自动注册）；NestJS 的 `@Module` 是刷卡进门，不显式声明 providers 谁都不放行。排查「Bean 没注入」时，Spring 查扫描路径、NestJS 查模块引用。
...
| 自定义 Provider（`useClass` / `useFactory` / 自定义 token + `@Inject('TOKEN')`） | `@Bean` + `@Qualifier` | 都是「手动注册 + 按名取用」；Spring 用 `@Bean` 方法返回实例、`@Qualifier` 限定注入哪个 |
| `@Injectable({scope: Scope.REQUEST})` | `@Scope("request")` | 都是「每次请求一个新实例」；Spring 作用域更全（singleton / prototype / request 等） |
| `forwardRef`（循环依赖） | `@Lazy` / ObjectProvider | 循环依赖的兜底；Spring 构造注入默认能解（三级缓存），少数场景才用 `@Lazy` / ObjectProvider 延迟获取 |
...
> NestJS 的 Guard ≈ Spring 的「Filter 链 / Interceptor / @PreAuthorize」**三者之一**——Spring 把能力拆成了更细的层次（见第 1 节的生命周期对照图），灵活，但「这段鉴权代码该放哪层」反而成了新决策（答：横切全部请求放过滤器链，MVC 层专属放拦截器，按方法角色放 @PreAuthorize）。
...
> 校验语义几乎一致；差异在**生态**：Bean Validation 是 JSR（Java Specification Request，Java 官方规范的编号流程）标准（跨 Hibernate Validator / 未来实现可换），而 class-validator 是 NestJS 生态专属件。另记一处分工：`ParseIntPipe` 这类参数级转换在 Spring 由 Converter（类型转换）+ Bean Validation（校验）两个机制分工完成。

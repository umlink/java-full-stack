...
> 本目录由 [java-fullstack-learning-roadmap.md](java-fullstack-learning-roadmap.md) 拆解而来。
...
整体学习分七个阶段外加起点盘点，由语言基础 → 框架生态 → 分布式架构 → 云原生运维 → 架构思维 → 全栈实战逐步推进。每个阶段都以前一阶段为前提，部分阶段可并行展开。
...
| 学习时间规划 | gantt 排期 + 主线 285 天说明 + 贯穿任务 | [08-学习时间规划.md](08-学习时间规划.md) |
| 配套学习资源清单 | 各阶段推荐书籍 / 官方文档汇总 | [09-配套学习资源清单.md](09-配套学习资源清单.md) |
| 学习建议与调整规则 | 学习方法论 + 企业招聘能力对标 | [10-学习建议与调整规则.md](10-学习建议与调整规则.md) |
| 学习进度追踪 | 进度打卡表 + 里程碑 + 偏差处理规则 | [11-学习进度追踪.md](11-学习进度追踪.md) |
| 源文档 | 拆解前的完整大纲（保留作对照） | [java-fullstack-learning-roadmap.md](java-fullstack-learning-roadmap.md) |

> ⚠️ **双份文档维护**：`java-fullstack-learning-roadmap.md` 为完整大纲快照，仅供对照总览；**日常学习一律以 docs/ 目录下的阶段拆分文件为准**。两者需同步改动时，先改拆分文件，再回写源大纲，避免漂移。

## 使用方式

1. 先读本页总览与 [专有名词速查表](00-专有名词速查表.md) 建立整体认知，再到 [阶段零](00-阶段零-起点盘点与补课/01-能力迁移对照.md) 盘点既有能力；用 [学习进度追踪](11-学习进度追踪.md) 开启打卡。
2. 按阶段顺序（零 → 七）逐个文件学习；每个文件含「精简大纲」「学习内容详情（带代码示例与名词解释）」「坑点提醒」「本节自检」「思考题」五部分。
3. 每个小点文件末尾附「本节自检」，达标后再进入下一个点。
4. 阶段 3 完成后即可启动项目 1（中后台系统），与阶段 4-5 的学习并行推进，边学边练（排期见 [学习时间规划](08-学习时间规划.md)）。
